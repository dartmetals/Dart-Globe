...
const Header: React.FC = () => {
  ...
  const [isPopupVisible, setIsPopupVisible] = useState(false);
  const [showMobileNotification, setShowMobileNotification] = useState(false);
  // NEW: controls the "Latest Update" poster popup. This is now the FIRST thing shown —
  // the register form (showPopup) only opens once the person taps "Claim Now" inside it.
...
useEffect(() => {
  ...
  if (!hasSeen) {
    const timer = setTimeout(() => {
      // Show the "Latest Update" poster popup first instead of jumping straight to the form.
...
const handlePopupClose = () => {
  ...
  // NEW: called when the "Latest Update" poster popup is dismissed (X button or backdrop tap)
  // WITHOUT claiming the offer. Mirrors handlePopupClose so the floating icon / mobile
  // notification still reappears for later.
  ...
  // NEW: called when "Claim Your Eligibility Assessment Today!" is tapped inside the
  // "Latest Update" poster popup. Closes that popup and immediately opens the register form.
            ...
            <div className="flex justify-between items-center">
              ...
              {scrolled && window.innerWidth < 1024 && showMobileNotification ? (
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => {
                      // Open the "Latest Update" poster popup, not the register form directly.
                      setShowUpdateModal(true);
                      setShowMobileNotification(false);
                    }}
                    className="w-8 h-8 rounded-full bg-gradient-to-r from-purple-600 to-blue-600 flex items-center justify-center hover:scale-110 transition-transform duration-300 cursor-pointer shadow-lg hover:shadow-xl"
...
{showMobileNotification && isMobile && !scrolled && (
  ...
  >
    <div className="max-w-7xl mx-auto px-4">
      ...
      {/* "Latest Update" poster popup — shown first. Tapping "Claim Now" inside it
          closes this and opens the register form below. */}
      <OfferUpdateModal
